...
- [Storm Terraform Modules Files](#storm-terraform-modules-files)
...
# Storm Terraform Modules Files

This package combines various base Terraform modules that are used in the Storm
...
```bash
pnpm add -D @storm-software/terraform-modules
```
...
```bash
npm install -D @storm-software/terraform-modules
```
...
```bash
yarn add -D @storm-software/terraform-modules
```
...
Run `nx test terraform-modules` to execute the unit tests via
