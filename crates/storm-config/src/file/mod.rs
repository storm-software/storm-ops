  ...
  F: FileStoredFormat + 'static,
{
  pub fn from_str(s: &str, format: F) -> Self {
    Self { format: Some(format), required: true, source: s.into() }
  }
}
...
impl File<source::file::FileSourceFile, FileFormat> {
  ...
  pub fn with_name(name: &str) -> Self {
    Self { format: None, required: true, source: source::file::FileSourceFile::new(name.into()) }
  }
}
...
impl From<PathBuf> for File<source::file::FileSourceFile, FileFormat> {
  fn from(path: PathBuf) -> Self {
    Self { format: None, required: true, source: source::file::FileSourceFile::new(path) }
  }
}
  ...
  T: Sync + Send + FileSource<F> + 'static,
{
  ...
  fn collect(&self) -> Result<Map<String, Value>> {
    ...
    let (uri, contents, format) =
      match self.source.resolve(self.format.clone()).map_err(ConfigError::Foreign) {
    ...
    format.parse(uri.as_ref(), &contents).map_err(|cause| ConfigError::FileParse { uri, cause })
  }
}
