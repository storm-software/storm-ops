...
This package contains shared [TSDoc](https://tsdoc.org/) configurations used in
Storm Software projects.
...
```bash
pnpm add -D @storm-software/tsdoc
```
...
```bash
npm install -D @storm-software/tsdoc
```
...
```bash
yarn add -D @storm-software/tsdoc
```
...
Run `nx build tsdoc` to build the library.

## Running unit tests

Run `nx test tsdoc` to execute the unit tests via [Jest](https://jestjs.io).
