import { Comparator, StringsRecord } from "conventional-changelog-writer";
import { DATETIME_LENGTH } from "./constants";

function hasIntersection(a: string | string[], b: string | string[]): boolean {
  if (!a || !b) {
    return false;
  }
...
export function matchScope(config: Config = {}, commit: Commit): boolean {
 ...
 */
export function createComparator<K extends string, T extends StringsRecord<K>>(
