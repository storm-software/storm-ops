...
/**
 * Creates the `storm-bun` command-line program.
 *
...
export function createProgram(config: StormWorkspaceConfig) {
  try {
    _config = config;
    writeInfo(
      `${brandIcon(_config)}  Running the \`storm-bun\` command-line application`,
      config
    );
    ...
    const program = new Command("storm-bun");
    program.version(
    ...
    program
      .command("update")
      .description("Update workspace catalog dependency package version.")
      .argument(
        "[packages...]",
...
async function updateAction(
  ...
  try {
    ...
    if (pkgs.length > 0) {
      ...
      await Promise.all(
        pkgs.map(async pkg => {
          ...
          if (matchedPackages.length === 0) {
            ...
            writeDebug(
              `${brandIcon(_config)}  Found ${matchedPackages.length} packages matching "${pkg}" in the workspace catalog file: \n\n- ${matchedPackages
      ...
      if (changed.length > 0) {
        writeDebug(
          "Finalizing changes to the workspace catalog dependencies",
          _config
        );
    ...
    if (nx) {
      writeDebug(
        "Running `bun nx migrate latest` to update Nx packages to their latest versions",
        _config
      );

      const proc = await runAsync(_config, "bun nx migrate latest");
    ...
    if (changed.length > 0) {
      if (install) {
        writeDebug(
          "Running `bun install` to update local dependency versions",
          _config
        );

        const proc = await runAsync(_config, "bun install");
        proc.stdout?.on("data", data => {
          console.log(data.toString());
        });
      }
