...
```bash
pnpm add -D @storm-software/terraform-tools
```
...
```bash
npm install -D @storm-software/terraform-tools
```
...
```bash
yarn add -D @storm-software/terraform-tools
```
...
Run `nx build terraform-tools` to build the library.

## Running unit tests

Run `nx test terraform-tools` to execute the unit tests via
