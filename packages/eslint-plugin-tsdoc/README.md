...
- [Storm Software - TSDoc Plugin for ESLint](#storm-software---tsdoc-plugin-for-eslint)
...
# Storm Software - TSDoc Plugin for ESLint

This package provides an ESLint plugin that enforces best practices and conventions for using TSDoc in TypeScript projects. It includes rules for ensuring that TSDoc comments are properly formatted, that all public APIs are documented, and that the documentation is consistent across the codebase. By using this plugin, developers can improve the quality and maintainability of their code documentation, making it easier for other developers to understand and use their code. The plugin is designed to be flexible and configurable, allowing teams to customize the rules to fit their specific needs and coding standards.
...
```bash
pnpm add -D @storm-software/eslint-plugin-tsdoc
```
...
```bash
npm install -D @storm-software/eslint-plugin-tsdoc
```
...
```bash
yarn add -D @storm-software/eslint-plugin-tsdoc
```
...
Run `nx build eslint-plugin-tsdoc` to build the library.

## Running unit tests

Run `nx test eslint-plugin-tsdoc` to execute the unit tests via [Jest](https://jestjs.io).
