...
# Storm TSDown Package

This package contains the TSDown configuration for Storm Software projects.
...
```bash
pnpm add -D @storm-software/tsdown
```
...
```bash
npm install -D @storm-software/tsdown
```
...
```bash
yarn add -D @storm-software/tsdown
```
...
Run `nx test tsdown` to execute the unit tests via [Jest](https://jestjs.io).
