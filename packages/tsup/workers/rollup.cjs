...
`);
    ...
    function getModule(
      ...
      if (existingProgram) {
        ...
        const newProgram = createProgram$1(
          fileName,
          compilerOptions,
          tsconfig,
          options
...
var tsResolvePlugin = ({ resolveOnly, ignore } = {}, options) => {
...
var getRollupConfig = async options => {
  ...
  const pkg = await _chunkVGC3FXLUjs.loadPkg.call(
    void 0,
    options.workspaceConfig?.workspaceRoot || process.cwd()
  );
    ...
    outputConfig: options.format.map(format => {
      const outputExtension =
        _optionalChain([
          options,
          "access",
          _17 => _17.outExtension,
          "optionalCall",
          _18 => _18({ format, options, pkgType: pkg.type }),
          "access",
          _19 => _19.dts
        ]) ||
  ...
  _27 =>
    _27("message", data => {
      ...
      const hasTypescript = _resolvefrom2.default.silent(
        data.options.workspaceConfig?.workspaceRoot || process.cwd(),
        "typescript"
      );
      if (!hasTypescript) {
