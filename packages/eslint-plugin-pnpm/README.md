...
- [Storm Software - PNPM Plugin for ESLint](#storm-software---pnpm-plugin-for-eslint)
...
# Storm Software - PNPM Plugin for ESLint

This package provides an ESLint plugin that enforces best practices and conventions for using PNPM in TypeScript projects. It includes rules for ensuring that PNPM configurations are properly formatted, that all dependencies are correctly specified, and that the workspace settings are consistent across the codebase. By using this plugin, developers can improve the quality and maintainability of their PNPM configurations, making it easier for other developers to understand and use their projects. The plugin is designed to be flexible and configurable, allowing teams to customize the rules to fit their specific needs and coding standards.
...
```bash
pnpm add -D @storm-software/eslint-plugin-pnpm
```
...
```bash
npm install -D @storm-software/eslint-plugin-pnpm
```
...
Run `nx build eslint-plugin-pnpm` to build the library.

## Running unit tests

Run `nx test eslint-plugin-pnpm` to execute the unit tests via [Jest](https://jestjs.io).
