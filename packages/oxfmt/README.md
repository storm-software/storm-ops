...
- [Storm Oxfmt Package](#storm-oxfmt-package)
...
This package contains the Oxfmt configuration for Storm Software projects.
...
```bash
pnpm add -D @storm-software/oxfmt
```
...
```bash
npm install -D @storm-software/oxfmt
```
...
```bash
yarn add -D @storm-software/oxfmt
```
...
Run `nx build oxfmt` to build the library.

## Running unit tests

Run `nx test oxfmt` to execute the unit tests via [Jest](https://jestjs.io).
