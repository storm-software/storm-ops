...
- [Storm Base Oxlint Package](#storm-base-oxlint-package)
...
# Storm Base Oxlint Package

This package contains the base Oxlint configurations used across the Storm
Software organization and in the organizations's Oxlint Plugin package. This
configuration is used to enforce code quality and style standards across all
...
```bash
pnpm add -D @storm-software/oxlint
```
...
```bash
npm install -D @storm-software/oxlint
```
...
```bash
yarn add -D @storm-software/oxlint
```
...
Run `nx test oxlint` to execute the unit tests via [Jest](https://jestjs.io).
