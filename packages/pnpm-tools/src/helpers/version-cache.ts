...
const VERSION_CACHE_TTL = 1000 * 60 * 60 * 2;
...
const storage = createStorage({
  driver: fsDriver({ base: envPaths("storm-pnpm").cache })
});
...
/**
 * Get an npm package version, using the Storm pnpm user cache for one day.
 *
...
export async function getCachedVersion(packageName: string, tag: string) {
  ...
  const version = await getVersion(packageName, tag, { executable: "pnpm" });
