import { fixSpaceAroundKeyword, replaceReferenceIdentifier } from "./fix/index";
import { GlobalReferenceTracker } from "./utils/global-reference-tracker";
import isLeftHandSide from "./utils/is-left-hand-side";
...
const messages = {
  ...
  [MESSAGE_ID_SUGGESTION]:
    "Replace `{{description}}` with `Number.{{property}}`."
};
...
function checkProperty({ node, path: [name] }, sourceCode) {
  ...
  const fix = fixer =>
    replaceReferenceIdentifier(node, `Number.${property}`, fixer, sourceCode);
