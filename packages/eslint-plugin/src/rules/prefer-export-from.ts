...
import { isStringLiteral } from "./ast/index";
...
function* removeSpecifier(node, fixer, sourceCode) {
  ...
  switch (node.type) {
    case "ImportSpecifier": {
      const hasOtherSpecifiers = specifiers.some(
        specifier => specifier !== node && specifier.type === node.type
      );
      if (!hasOtherSpecifiers) {
        const closingBraceToken = sourceCode.getTokenAfter(
          node,
          isClosingBraceToken
        );
        ...
        yield fixer.replaceTextRange(
          [commaToken.range[0], closingBraceToken.range[1]],
          ""
        );
        return;
      }
