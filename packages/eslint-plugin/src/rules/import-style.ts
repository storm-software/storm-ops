...
const create = context => {
  ...
  const report = (
    ...
    // For `require`, `'default'` style allows both `x from 'x')` (`'namespace'` style) and
    // `{default: x} from 'x')` (`'default'` style) since we don't know in advance
    ...
    if (
      actualImportStyles.every(style => effectiveAllowedImportStyles.has(style))
    ) {
      return;
    }

    const data = {
      allowedStyles: new Intl.ListFormat("en-US", {
        type: "disjunction"
      }).format([...allowedImportStyles.keys()]),
      moduleName
    };
