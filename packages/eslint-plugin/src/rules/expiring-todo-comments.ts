...
import getBuiltinRule from "./utils/get-builtin-rule";
...
function getPackageHelpers(directoryName: string) {
  ...
  let packageResult!: ReturnType<typeof readPackageUpSync>;
  try {
    packageResult = readPackageUpSync({ normalize: false, cwd: directoryName });
  } catch {
  ...
  function parseTodoWithArguments(string, { terms }) {
    ...
    const parsedArguments = result.groups?.rawArguments
      ?.split(",")
      .map(argument => parseArgument(argument.trim(), directoryName));

    return createArgumentGroup(parsedArguments);
  }

  function parseArgument(argumentString, dirname = directoryName) {
    ...
    if (hasPackage && VERSION_COMPARISON_RE.test(argumentString)) {
      const versionComparison = VERSION_COMPARISON_RE.exec(argumentString);
      if (versionComparison?.groups) {
        const groups = versionComparison?.groups;
...
const create = context => {
  ...
  function processComment(comment) {
    ...
    const parsed = parseTodoWithArguments(comment.value, options) as any;
    if (!parsed) {
      return true;
    }
