...
@returns {boolean}
*/
export function shouldAddParenthesesToLogicalExpressionChild(
  node,
  { operator, property }
) {
  ...
  if (node.type === "AwaitExpression" || node.type === "BinaryExpression") {
    return true;
  }
...
export default shouldAddParenthesesToLogicalExpressionChild;
