...
export type TraceMap = Record<string, boolean | Record<string, any>>;

export const createTraceMap = (object, type) => {
  let map = { [type]: true } as TraceMap;

  const path = object.split(".").reverse();
  for (const name of path) {
    map = { [name]: map } as TraceMap;
  }
