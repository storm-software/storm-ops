export type AssertTokenOptions = {
  test?: any;
  expected: any;
  ruleId: any;
};

const ISSUE_LINK_PREFIX =
  "https://github.com/storm-software/storm-ops/issues/new?";
export function assertToken(
  token,
  { test, expected, ruleId }: AssertTokenOptions
) {
  if (test?.(token)) {
    return;
  }
  ...
  expected = expected.map(expectedToken =>
    typeof expectedToken === "string" ? { value: expectedToken } : expectedToken
  );

  if (
    !test &&
    expected.some(expectedToken =>
      Object.entries(expectedToken).every(
  ...
  expected = expected
    .map(expectedToken => `'${JSON.stringify(expectedToken)}'`)
    .join(" or ");
