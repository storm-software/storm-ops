import { findVariable, isParenthesized } from "@eslint-community/eslint-utils";
...
const create = context => {
  ...
  context.on("VariableDeclarator", node => {
    ...
    const zeroIndexNodes = [] as any[];
    const destructuringNodes = [] as any[];
  ...
  if (!createOptions?.checkFromLast) {
    return;
  }
