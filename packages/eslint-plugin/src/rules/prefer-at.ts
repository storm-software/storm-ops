...
import { isCallExpression, isLiteral, isMethodCall } from "./ast/index";
import { removeMemberExpressionProperty, removeMethodCall } from "./fix/index";
import {
  getNegativeIndexLengthNode,
  removeLengthNode
} from "./shared/negative-index";
import {
...
const messages = {
  ...
  [MESSAGE_ID_STRING_CHAR_AT_NEGATIVE]:
  ...
  [MESSAGE_ID_GET_LAST_FUNCTION]:
    "Prefer `.at(-1)` over `{{description}}(…)` to get the last element.",
...
const lodashLastFunctions = ["_.last", "lodash.last", "underscore.last"];
...
function create(context) {
  ...
  const getLastFunctions = [
    ...createOptions.getLastElementFunctions,
    ...lodashLastFunctions
  ];
  ...
  context.on("MemberExpression", node => {
    if (node.optional || !node.computed || isLeftHandSide(node)) {
      return;
    }
    ...
    if (!lengthNode) {
      if (!createOptions.checkAllIndexAccess) {
        return;
      }
    ...
    problem.fix = function* (fixer) {
      ...
      if (
        ...
        if (
          ...
          tokenBefore.value === "-" &&
          /^\s+$/.test(
            sourceCode.text.slice(tokenBefore.range[1], numberNode.range[0])
          )
        ) {
  ...
  context.on("CallExpression", node => {
    ...
    if (!lengthNode && !createOptions.checkAllIndexAccess) {
      return;
    }
