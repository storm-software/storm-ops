import { isFunction, isReferenceIdentifier, isStaticRequire } from "./ast";
import {
  removeParentheses,
  removeSpacesAfter,
  replaceReferenceIdentifier
} from "./fix";
import { getCallExpressionTokens } from "./utils";
...
function create(context) {
  ...
  context.on("ExpressionStatement", node => {
    ...
    const problem = { node, messageId: ERROR_USE_STRICT_DIRECTIVE } as any;
  ...
  context.on("ReturnStatement", (node: any) => {
