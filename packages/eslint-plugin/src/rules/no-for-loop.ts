...
import { isLiteral } from "./ast/index";
...
import toLocation from "./utils/to-location";
...
const create = context => {
  ...
  return {
    ForStatement(node) {
      ...
      const [, end] = sourceCode.getTokenBefore(
        node.body,
        isClosingParenToken
      ).range;
          ...
          if (elementNode) {
            ...
            if (removeDeclaration) {
              ...
              if (elementNode.id.typeAnnotation && shouldGenerateIndex) {
                declarationElement = sourceCodeText
                  .slice(
                    elementNode.id.range[0],
                    elementNode.id.typeAnnotation.range[0]
                  )
                  .trim();
                typeAnnotation = sourceCode
                  .getText(
