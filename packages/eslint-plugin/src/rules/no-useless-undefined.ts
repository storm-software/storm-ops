...
import { isFunction, isUndefined } from "./ast/index";
import { replaceNodeOrTokenAndSpacesBefore } from "./fix/index";
...
const create = context => {
  ...
  context.on("Identifier", node => {
    if (
      ...
      return getProblem(node, fixer => removeNodeAndLeadingSpace(node, fixer));
    }
  });
