import { isLiteral, isMethodCall } from "./ast/index";
import {
  removeParentheses,
...
function createLogCallTimesConstantCheck({ constantName, replacementMethod }) {
  ...
  return function (node, context) {
    ...
    if (
      ...
      isMathMethodCall(node.right, "log") &&
      isMathProperty(node.left, constantName)
    ) {
    ...
    return {
      ...
      fix: fixer =>
        fixer.replaceText(
          node,
          `Math.${replacementMethod}(${getParenthesizedText(valueNode, context.sourceCode)})`
        )
    };
  };
}
...
const checkFunctions = [
  createLogCallTimesConstantCheck({
    constantName: "LOG10E",
    replacementMethod: "log10"
  }),
  createLogCallTimesConstantCheck({
    constantName: "LOG2E",
    replacementMethod: "log2"
  }),
  createLogCallDivideConstantCheck({
    constantName: "LN10",
    replacementMethod: "log10"
  }),
  createLogCallDivideConstantCheck({
...
const create = context => {
  ...
  return {
    CallExpression(callExpression) {
      ...
      const plusExpressions = new Set(
        expressions.length === 1
          ? []
          : expressions.map(expression => expression.parent)
      );
      ...
      return {
        ...
        *fix(fixer) {
          ...
          for (const expression of plusExpressions) {
            const plusToken = sourceCode.getTokenAfter(
              expression.left,
