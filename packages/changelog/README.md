...
[![Version](https://img.shields.io/badge/version-1.28.5-1fb2a6.svg?style=for-the-badge&color=1fb2a6)](https://prettier.io/)&nbsp;
...
# Storm Release CHANGELOG File Renderer

This package contains a tool that will generate a `CHANGELOG.md` file for a project. It will use the commit messages to generate the file, and will also include the version number and date of the release.
...
```bash
pnpm add -D @storm-software/changelog
```
...
```bash
npm install -D @storm-software/changelog
```
...
```bash
yarn add -D @storm-software/changelog
```
...
Run `nx build changelog` to build the library.

## Running unit tests

Run `nx test changelog` to execute the unit tests via [Jest](https://jestjs.io).
