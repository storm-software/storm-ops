...
export class StormNxWorkspace extends NxWorkspace {
  ...
  public override namedInputs: Nx.INamedInputs = {
    ...
    linting: [
      ...
      "{workspaceRoot}/.markdownlint-cli2.@(js|cjs|mjs|ts|cts|mts)",
      ...
      "{projectRoot}/.markdownlint.json",
      "{projectRoot}/.markdownlint-cli2.@(js|cjs|mjs|ts|cts|mts)",
      "{projectRoot}/.prettier@(rc|ignore)"
    ],
    documentation: [
      "sharedGlobals",
    ...
    typescript: [
      ...
      "{projectRoot}/{build,tsup,contentlayer}.*.[jt]s?(x)",
