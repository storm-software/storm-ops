  ...
  git-hooks = {
    ...
    hooks = {
      eslint = {
        ...
        entry = "pnpm eslint --fix --color --concurrency 5 --no-warn-ignored --max-warnings -1 --cache --cache-location \"${config.env.DEVENV_ROOT}/node_modules/.cache/storm/eslint-cache\" --config \"${config.env.DEVENV_ROOT}/eslint.config.mjs\" ";
        files = "";
        excludes = [
